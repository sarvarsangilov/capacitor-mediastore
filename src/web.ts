...
function generateMockAudio(count: number): MediaItem[] {
...
function readRecentStore(): PickedFile[] {
  if (typeof window === 'undefined' || !window.localStorage) return [];
  try {
    const raw = window.localStorage.getItem(STORE_KEY);
    return raw ? (JSON.parse(raw) as PickedFile[]) : [];
  } catch {
    return [];
  }
}

function writeRecentStore(items: PickedFile[]): void {
  if (typeof window === 'undefined' || !window.localStorage) return;
  try {
...
export class CapacitorMediastoreWeb extends WebPlugin implements CapacitorMediastorePlugin {
  ...
  async getRecentFiles(options?: GetRecentFilesOptions): Promise<GetRecentFilesResult> {
    const limit = options?.limit ?? 50;
    const offset = options?.offset ?? 0;
    const mimes = options?.mimeTypes ?? [];
  ...
  async resolveRecentFile(options: ResolveRecentFileOptions): Promise<ResolveRecentFileResult> {
    const all = readRecentStore();
    const idx = all.findIndex((f) => f.id === options.id);
    if (idx < 0) return { file: null };
    const updated: PickedFile = { ...all[idx], lastAccessedAt: new Date().toISOString() };
    all[idx] = updated;
